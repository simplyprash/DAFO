...
import {
  DafoAuctionHouse,
  DafoAuctionHouse__factory as DafoAuctionHouseFactory,
  DafoDescriptor,
...
async function deploy() {
  [deployer, bidderA, wethDeployer, dafoundersDAO] = await ethers.getSigners();
  ...
  dafoToken = await deployDafoToken(
    deployer,
  ...
  const dafoAuctionHouseProxy = await upgrades.deployProxy(auctionHouseFactory, [
    dafoToken.address,
    weth.address,
    TIME_BUFFER,
    RESERVE_PRICE,
  ...
  dafoAuctionHouse = DafoAuctionHouseFactory.connect(dafoAuctionHouseProxy.address, deployer);

  // 3. SET MINTER
  await dafoToken.setMinter(dafoAuctionHouse.address);

  // 4. POPULATE body parts
  descriptor = DafoDescriptorFactory.connect(await dafoToken.descriptor(), deployer);
  ...
  timelock = await new NounsDaoExecutorFactory(deployer).deploy(calculatedGovDelegatorAddress, TIME_LOCK_DELAY);
  ...
  const nounsDAOProxy = await new NounsDaoProxyFactory(deployer).deploy(
    timelock.address,
    dafoToken.address,
    dafoundersDAO.address, // NoundersDAO is vetoer
    timelock.address,
    govDelegate.address,
    VOTING_PERIOD,
    VOTING_DELAY,
  ...
  await dafoToken.transferOwnership(timelock.address);
  ...
  await dafoAuctionHouse.unpause();

  // 11. SET Auction House owner to NounsDAOExecutor
  await dafoAuctionHouse.transferOwnership(timelock.address);
}
...
describe('End to End test with deployment, auction, proposing, voting, executing', async () => {
  ...
  it('sets all starting params correctly', async () => {
    expect(await dafoToken.owner()).to.equal(timelock.address);
    expect(await descriptor.owner()).to.equal(timelock.address);
    expect(await dafoAuctionHouse.owner()).to.equal(timelock.address);

    expect(await dafoToken.minter()).to.equal(dafoAuctionHouse.address);
    expect(await dafoToken.dafoundersDAO()).to.equal(dafoundersDAO.address);
    ...
    expect(await gov.vetoer()).to.equal(dafoundersDAO.address);

    expect(await dafoToken.totalSupply()).to.equal(EthersBN.from('2'));

    expect(await dafoToken.ownerOf(0)).to.equal(dafoundersDAO.address);
    expect(await dafoToken.ownerOf(1)).to.equal(dafoAuctionHouse.address);

    expect((await dafoAuctionHouse.auction()).customInput.tokenId).to.equal(EthersBN.from('1'));
  });
  ...
  it('allows bidding, settling, and transferring ETH correctly', async () => {
    ...
    await dafoAuctionHouse.connect(bidderA).createBid(customInput, { value: RESERVE_PRICE });
    await setNextBlockTimestamp(Number(await blockTimestamp('latest')) + DURATION);
    await dafoAuctionHouse.settleCurrentAndCreateNewAuction();

    expect(await dafoToken.ownerOf(1)).to.equal(bidderA.address);
  ...
  it('allows proposing, voting, queuing', async () => {
    ...
    targets.push(dafoToken.address);
    values.push('0');
  ...
  it('executes proposal transactions correctly', async () => {
    ...
    expect(await dafoToken.minter()).to.equal(address(1));

...
describe('Nouns Governance', () => {
  ...
  let token: DafoToken;
  let customizer: DafoCustomizer;
  let tokenCallFromGuy: DafoToken;
  let tokenCallFromDeployer: DafoToken;
  ...
  before(async () => {
    ...
    token = await deployDafoToken(signers.deployer);

    await populateDescriptor(DafoDescriptorFactory.connect(await token.descriptor(), signers.deployer));
  ...
  describe('delegateBySig', () => {
    ...
    it('reverts if the signatory is invalid', async () => {
      ...
      await expect(token.delegateBySig(delegatee, nonce, expiry, 0, badhex, badhex)).to.be.revertedWith(
  ...
  describe('numCheckpoints', () => {
    ...
    it('returns the number of checkpoints for a delegate', async () => {
      await setTotalSupply(token, customizer, 3);
    ...
    it('does not add more than one checkpoint in a block', async () => {
      await setTotalSupply(token, customizer, 4);
  ...
  describe('getPriorVotes', () => {
    ...
    it('returns the latest block if >= last checkpoint block', async () => {
      await setTotalSupply(token, customizer, 1);
    ...
    it('generally returns the voting balance at the appropriate checkpoint', async () => {
      ...
      const t2 = await (await tokenCallFromDeployer.transferFrom(deployer.address, account0.address, 0)).wait();
      await mineBlock();
      await mineBlock();

      // deployer -> account0.address id 2
      const t3 = await (await tokenCallFromDeployer.transferFrom(deployer.address, account0.address, 1)).wait();
      await mineBlock();
      await mineBlock();

      // account0.address -> deployer id 1
      const t4 = await (await tokenCallFromGuy.transferFrom(account0.address, deployer.address, 0)).wait();
      await mineBlock();
      await mineBlock();

...
async function reset(): Promise<void> {
  ...
  // nonce 3: Deploy DafoDescriptor
  // nonce 4: Deploy DafoCustomizer
  // nonce 5: Deploy DafoToken
  ...
  // Deploy Nouns token
  token = await deployDafoToken(deployer);
  customizer = await deployDafoCustomizer(deployer);
  ...
  await populateDescriptor(DafoDescriptorFactory.connect(await token.descriptor(), deployer));
...
describe('NounsDAO#vetoing', () => {
  ...
  it('rejects setting a new vetoer when sender is not vetoer', async () => {
    await expect(gov.connect(account0)._setVetoer(account1.address)).revertedWith('NounsDAO::_setVetoer: vetoer only');
  });
  ...
  it('burns veto power correctly', async () => {
    ...
    await expect(gov.connect(vetoer).veto(proposalId)).revertedWith('NounsDAO::veto: veto power burned');
  });
  ...
  describe('vetoing works correctly for proposal state', async () => {
    ...
    it('Executed', async () => {
      ...
      await expect(gov.veto(proposalId)).revertedWith('NounsDAO::veto: cannot veto executed proposal');
    });
    it('Vetoed', async () => {
      await propose(account0);
      await expectState(proposalId, 'Pending');

...
const states: string[] = ['Pending', 'Active', 'Canceled', 'Defeated', 'Succeeded', 'Queued', 'Expired', 'Executed'];
...
async function makeProposal(
  ...
  await setTotalSupply(token, customizer, mintAmount);
...
describe('NounsDAO#state/1', () => {
  before(async () => {
    ...
    customizer = await deployDafoCustomizer(signers.deployer);
  ...
  it('Cannot queue if defeated', async () => {
    ...
    await expect(gov.queue(proposalId)).revertedWith('queue: proposal can only be queued if it is succeeded');
  });
  ...
  it('Executed, only after queued', async () => {
    ...
    await expect(gov.execute(proposalId)).revertedWith('execute: proposal can only be executed if it is queued');

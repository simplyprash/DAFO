...
async function reset(): Promise<void> {
  ...
  seeder = await deployDafoCustomizer(deployer);
...
describe('NounsDAO#inflationHandling', () => {
  ...
  it('returns quorum votes and proposal threshold based on Noun total supply', async () => {
    // Total Supply = 40
    await setTotalSupply(token, seeder, 40);
  ...
  it('rejects if proposing below threshold', async () => {
    ...
    await mineBlock();
    await expect(gov.connect(account0).propose(targets, values, signatures, callDatas, 'do nothing')).revertedWith(
  ...
  it('returns updated quorum votes and proposal threshold when total supply changes', async () => {
    // Total Supply = 80
    await setTotalSupply(token, seeder, 80);
  ...
  it('rejects proposals that were previously above proposal threshold, but due to increasing supply are now below', async () => {
    ...
    await expect(gov.connect(account1).propose(targets, values, signatures, callDatas, 'do nothing')).revertedWith(
      'NounsDAO::propose: proposer votes below proposal threshold'
    );
  });

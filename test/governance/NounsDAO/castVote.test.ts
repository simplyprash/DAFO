...
async function deployGovernor(deployer: SignerWithAddress, tokenAddress: string): Promise<NounsDaoLogicV1Harness> {
  const { address: govDelegateAddress } = await new NounsDaoLogicV1HarnessFactory(deployer).deploy();
...
describe('NounsDAO#castVote/2', () => {
  ...
  describe('We must revert if:', () => {
    ...
    it("There does not exist a proposal with matching proposal id where the current block number is between the proposal's start block (exclusive) and end block (inclusive)", async () => {
      await expect(gov.castVote(proposalId, 1)).revertedWith('NounsDAO::castVoteInternal: voting is closed');
    });

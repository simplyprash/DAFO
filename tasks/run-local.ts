...
task('run-local', 'Start a hardhat node, deploy contracts, and execute setup transactions').setAction(
  async (_, { ethers, run }) => {
    ...
    await run('populate-descriptor', {
      ...
      dafoDescriptor: contracts.DafoDescriptor.instance.address,
    });

    await contracts.DafoAuctionHouse.instance.attach(contracts.DafoAuctionHouseProxy.instance.address).unpause({
      gasLimit: 1_000_000,
    });
    ...
    console.log(`Dafo contracts deployed to local node at http://localhost:8545 (Chain ID: ${chainId})`);
    console.log(`Auction House Proxy address: ${contracts.DafoAuctionHouseProxy.instance.address}`);
    console.log(`Dafos ERC721 address: ${contracts.NounsToken.instance.address}`);

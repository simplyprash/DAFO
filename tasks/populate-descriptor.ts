...
task('populate-descriptor', 'Populates the descriptor with color palettes and Noun parts')
  ...
  .addOptionalParam(
    'dafoDescriptor',
    'The `DafoDescriptor` contract address',
    '0xe7f1725E7734CE288F8367e1Bb143E90bb3F0512',
    types.string
  )
  .setAction(async ({ nftDescriptor, dafoDescriptor }, { ethers }) => {
    const descriptorFactory = await ethers.getContractFactory('DafoDescriptor', {
      libraries: {
        NFTDescriptor: nftDescriptor,
      },
    });
    const descriptorContract = descriptorFactory.attach(dafoDescriptor);
